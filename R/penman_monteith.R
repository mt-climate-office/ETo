...
etr_penman_monteith <- function(lat, days, rh, tmean, srad, ws, elev, reference = 0.23) {
  # checkmate::assert_multi_class(
  #   c(lat, days, rh, tmean, srad, ws, elev, reference),
  ...
  sat_vapor_pressure <- calc_sat_vapor_pressure(tmean)
  ...
  rad_lw <- calc_longwave_radiation(tmean, actual_vapor_pressure, radiation_fraction)
  rad_sw <- calc_shortwave_radiation(rad_in, reference)
  rad_net <- calc_net_radiation(rad_sw, rad_lw)
  svp_slope <- calc_svp_slope(tmean)
  pressure <- calc_pressure(elev)
  psy <- calc_psychrometric_constant(pressure)

  ((0.408 * svp_slope * (rad_net - 0)) + (psy * (900 / (tmean + 273)) * (ws * (sat_vapor_pressure - actual_vapor_pressure)))) /
